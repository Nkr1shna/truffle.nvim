...
-- Internal: send data to terminal; forwards raw data via chansend without mutation
...
local function create_default_keymaps()
	...
	apply_mapping("toggle", "n", mappings.toggle, function()
		M.toggle()
	end, "Truffle: Toggle panel")
	apply_mapping("send_selection", "v", mappings.send_selection, function()
		M.send_visual()
	end, "Truffle: Send selection")
	apply_mapping("send_file", "n", mappings.send_file, function()
		M.send_file({ path = "current" })
